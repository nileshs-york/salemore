            ...
            <div className="hidden md:flex items-center gap-12">
              {navLinks.map((link) => (
                <div key={link.name} className="relative" onMouseEnter={() => link.hasDropdown && setIsCollectionsOpen(true)}>
                  <Link
                    to={link.path}
                    className={`text-[10px] font-bold uppercase tracking-[0.2em] transition-all hover:text-brand-accent relative group text-white ${
                      location.pathname === link.path ? 'text-brand-accent' : ''
                    }`}
                  >
                    {link.name}
                    <span className={`absolute -bottom-2 left-0 h-[2px] bg-brand-accent transition-all ${
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="grid grid-cols-1 md:grid-cols-12 gap-20 mb-24">
            ...
            <div className="md:col-span-3 space-y-8">
              ...
              <ul className="space-y-4">
                {navLinks.map((link) => (
                  <li key={link.name}>
                    <Link to={link.path} className="text-white/60 hover:text-white transition-colors text-sm font-medium">
                      {link.name}
                    </Link>
                  </li>
                ))}
          ...
          <div className="pt-12 border-t border-white/10 flex flex-col items-center gap-4">
            <p className="text-white/30 text-[10px] font-bold uppercase tracking-widest text-center">
              © {new Date().getFullYear()} Salemore. All rights reserved. | Website crafted with precision by <a href="https://strapnova.com" target="_blank" rel="noopener noreferrer" className="hover:text-white transition-colors">StrapNova</a>
            </p>
          </div>
        </div>
      </footer>
